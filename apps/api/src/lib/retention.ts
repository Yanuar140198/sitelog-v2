...
export async function pruneAuditLog(daysToKeep = 365): Promise<{ deletedAudit: number; deletedDeliveries: number }> {
  ...
  const audit = await db.delete(auditLog).where(lt(auditLog.createdAt, cutoff)).returning({ id: auditLog.id });
  // Also prune webhook deliveries older than 30 days
  const webhookCutoff = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  const dels = await db.delete(webhookDelivery).where(lt(webhookDelivery.createdAt, webhookCutoff)).returning({ id: webhookDelivery.id });
